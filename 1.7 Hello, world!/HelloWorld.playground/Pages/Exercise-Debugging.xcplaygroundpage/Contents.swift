/*:
 ...
 Tracking down code issues, or _bugs_, is one of the most common uses of the console. As it turns out, programmers spend a lot of time dealing with code that doesn’t yet work. If it worked just the way they wanted it to, they wouldn’t still be revising it.
 ...
 Let’s imagine your friends have done something that has completely shocked you, and you've decided to send each one a personalized message expressing your feelings. You don’t want to take the time to write individual messages, so you’ve created a program to generate a whole batch of them. Once it’s working, you’re pretty sure you can just change out the `name` variable, then copy and paste your pseudo-personalized shocked reaction to each of your friends. */
...
/*:
 ...
 Your result won’t look like a typical text message yet. It still has some odd punctuation and even some code-looking bits. You could fix these by closely inspecting the code and changing things until the final result looks right, but it’ll be easier if you can examine some of the intermediate constants along the way.
 ...
 4. Once the final string looks like something a human would send, play around with the constants to create your own message:
